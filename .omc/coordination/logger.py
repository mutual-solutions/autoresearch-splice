...
injection:
    ...
    import sys
    sys.path.insert(0, ".omc/coordination")
    from logger import get_logger

    log = get_logger("detector.gbm")
    log.emit("INFO", "diag.gbm.dedupe", before=120, after=80)

Any invocation must go through direct file paths (`uv run python .omc/...`).
`python -m omc.coordination.*` DOES NOT work and is not a supported mode.
